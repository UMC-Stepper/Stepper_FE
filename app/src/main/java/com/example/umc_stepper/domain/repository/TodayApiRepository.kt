...
interface TodayApiRepository {
    ...
    suspend fun getMyExercise(bodyPart: String) : Flow<BaseListResponse<CheckExerciseResponse>>     // 나만의 운동 조회 -> 스크랩 화면
    suspend fun getExerciseMonthCheck(month: Int) : Flow<BaseListResponse<ExerciseCardStatusResponseDto>> // 월별 운동 카드 상태 조회 API
    suspend fun postAddMyExercise(addExerciseRequestDto: AddExerciseRequestDto) : Flow<BaseResponse<AddExerciseResponse>>  // 나만의 운동 추가 API
    suspend fun getExerciseCheckDate(bodyPart: String) : Flow<BaseListResponse<ExerciseCardWeekResponseDto>> // 운동 부위의 운동 카드 요일 조회
}